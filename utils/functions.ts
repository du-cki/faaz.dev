...
const getRecentProjects = async (): Promise<Project[]> => {
  const req = await fetch('https://api.github.com/users/du-cki/repos?type=owner&sort=updated&per_page=3')

  if (!req.ok) {
    throw new Error(
        `Could not react GitHub, request returned a ${req.status}`
    )
  }
  ...
  return resp.map((data: ListRepositoryPayload) => ({
    ...
    color:
        languageColourMapping[
          data.language!.toLowerCase().replace(' ', '_') as keyof typeof languageColourMapping
